<a href="{{url('admin/dashboard')}}" title="Go to Dahboard" class="operation pull-left"><span class="glyphicon glyphicon-chevron-left"></span></a><br>
        @if(count($noofpost))
        <h4>Post not yet verified</h4>
        <div class="table-responsive">
          <table class="table">
            <tr>
              <th class="">Author</td>
              <th class="">Blog Title</td>
              <th class="">Posted On</td>
              <th class="">Operation</td>
            </tr>
            @foreach($noofpost as $post)
              <tr>
                <td>{{$post->firstname.' '.$post->lastname}}</td>
                <td>{{$post->title}}</td>
                <td>{{$post->postedon}}</td>
                <td>
                    <a href="#{{url('admin/delete/'.$post->blogid)}}" data-id="{{$post->blogid}}" title="Delete This Post" class="operation deletepost"><span class="glyphicon glyphicon-remove-circle"></span></a>
                    <a href="#{{url('admin/verify/'.$post->blogid)}}" data-id="{{$post->blogid}}" title="Mark as Verified" class="operation verifypost"><span class="glyphicon glyphicon-ok-circle"></span></a>
                    <a href="{{url('admin/blogdisplay/'.$post->blogid)}}" data-id="{{$post->blogid}}" target="_blank" style="color:#fff">View</a>
                </td>
              </tr>
            @endforeach
          </table>
        </div>
        <?php echo $noofpost->links(); ?>
        @else
          <span>No new Post available</span>
        @endif